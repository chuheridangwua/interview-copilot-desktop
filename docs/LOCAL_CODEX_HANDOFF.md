...
- 通用问题库内置到客户端；面试前可选 `resources/company/<公司名>` 下的公司资料和公司题库。选择公司后先匹配公司题库，公司题库没有候选时才匹配通用题库。
- 手动标记是当前推荐主路径：运行中点击 `标记问题` 或按 `M` 开始，再次点击或按 `M` 结束；提交时会取开始前 10 秒到结束之间的系统 ASR 文本，重组问题、匹配题库并生成流式口述稿。无论匹配分数高低，最终答案都由 AI 生成；AI 可以一模一样输出题库答案，也可以结合当前问法改写。
...
- `resources/company/<公司名>/Introduction.md` 用作公司背景和岗位上下文，`question.md` 会在选择公司后作为本场公司优先 matcher。
...
- 面试前选择公司后，题库健康状态、候选来源徽标、公司优先匹配/通用兜底和模型口述稿是否正确结合公司资料。
...
9. 推荐在面试官开始问问题时点击 `标记问题` 或按 `M`，问题结束后再次点击或按 `M`。系统会补入开始前 10 秒系统 ASR，并立即整理问题、匹配候选、生成答案；选择公司后先查公司题库，没有公司候选时才查通用题库。
10. 无论 Top1 题库匹配分数高低，答案都必须由 AI 生成；AI 会收到题库候选、简历、公司资料和上下文，可以选择一模一样输出题库答案，也可以结合当前问法改写。
11. 标记中可点 `取消`；最近一条手动问题可点 `撤销手动`。撤销会从当前 UI 和快照文件中移除，并追加审计事件。
12. 未手动标记时，自动抽题仍会作为兜底结合最近面试官转写和候选人上下文抽取问题；手动标记期间自动 final 不进入历史。
13. 右侧语音识别列同时显示系统实时/系统转写和麦克风实时/麦克风转写。
14. 按空格可在运行和暂停之间切换；点击 `结束面试` 会停止采集并写入本场归档。运行或暂停时不能切换公司。
...
其中 `system-transcript.*` 是面试官系统声音识别文字，`microphone-transcript.*` 是本人麦克风文字，`combined-transcript.*` 按时间合并两路文字。`question-answers.md` 按问题保存匹配到的题库答案和 AI 生成的最终输出答案。手动问题会记录 `source: "manual_marker"`、标记起止时间和手动片段；撤销手动问题会追加 `manual_question_undone` 事件。

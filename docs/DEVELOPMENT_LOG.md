...
明确答案生成边界：无论题库匹配分数高低，最终答案都必须由 AI 再生成一遍；AI 可以一模一样输出题库答案，也可以结合当前问法改写。同步把匹配到的题库候选列表从左下移动到第二列 `匹配原文答案` 面板顶部，并把右下 `面试公司` 原生 select 改成应用内下拉菜单，避免 Electron 原生下拉偶发不弹出。

### 已完成

- `maybeGenerateFallbackAnswer()` 不再按匹配分数短路；每个 final 问题都会调用方舟生成输出答案。
- 方舟生成提示词明确：
  - 无论题库匹配分数高低，都必须生成一版可直接口述的答案。
  - 题库候选明显匹配时，可以一模一样输出题库答案。
  - 也可以在不改变事实、立场和核心结构的前提下，改写成更适合现场口述的回答。
- Top1 候选会传入更长的 `answerLogic` / `answerDetail` 片段，便于模型必要时直接复述题库答案。
- 前端移除左下 `匹配到的问题` 面板，把 Top3 题库候选列表放到第二列 `匹配原文答案` 面板顶部。
- 左侧 `面试官问题列表` 改为占满整列；第二列顶部候选列表保持紧凑滚动，不挤掉下方题库原文答案。
- 右侧 `输出答案` 恢复为 Mini / Pro Fast 两个 AI 生成卡片，不再出现“题库命中，未调用模型”的短路卡片。
- 右下 `面试公司` 控件从原生 `<select>` 改为自绘按钮和上弹菜单：
  - 点击整个控件区域即可打开。
  - 点击外部或按 Esc 关闭。
  - 运行/暂停面试时仍自动关闭并禁止切换公司。
- README 和 `docs/LOCAL_CODEX_HANDOFF.md` 已同步“始终 AI 生成”和新布局说明。
...
- 当前环境未安装 Playwright，未做浏览器截图自动化验证；本次以构建和静态布局检查为准。
- 如果后续发现 AI 对高分题库命中改写过度，可以继续加强提示词为“高分题库命中优先逐字复述”。

### 下一步

- Windows Electron 真机跑一段面试音频，确认高分题库命中时仍会生成 Mini / Pro 输出，并确认公司菜单不再出现需要切换窗口后才能弹出的问题。

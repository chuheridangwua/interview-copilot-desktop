...
- 面试前可选择公司，匹配时先查该公司题库；公司题库没有候选时再查通用题库，并把公司资料注入模型口述稿。
- 本地毫秒级匹配题库候选，界面显示公司优先/通用兜底后的 Top 3；小模型后台确认问题、重排候选。无论匹配分数高低，最终答案都由 AI 生成，AI 可以直接一模一样输出题库答案，也可以结合当前问法改写。
...
`Introduction.md` 会作为公司背景、岗位 JD 和面试重点上下文注入模型口述稿；`question.md` 沿用基础题库格式，会在选择该公司后作为本场公司优先匹配题库。目录名就是公司下拉里的展示名，例如 `resources/company/数美` 会显示为 `数美`。
...
15. 左侧问题记录新增后，第二列 `匹配原文答案` 顶部显示 Top 3 题库匹配，下面显示当前选中题库原文答案，右侧显示最终输出答案；选择公司后先匹配该公司题库，没有公司候选时才显示通用题库候选。
